...
selectFinalSignatureGenes <- function(top_genes_per_patient) {
  ...
  # Sort by count (descending) then by avg_logfc_rank (ascending)
...
analyzeCorrelationsInDrugA <- function(drug_a_data, select_omics, drug_b_scores) {
  ...
  for (patient_id in names(drug_a_data)) {
    ...
    for (group in c("Response", "Non_response")) {
      ...
      # Get B drug scores for the same samples
      # Use average B score from all datasets as reference
      all_b_scores <- unlist(lapply(drug_b_scores, function(x) x$scores))
      ref_b_score <- mean(all_b_scores, na.rm = TRUE)

      # Calculate correlation between omics expression and B drug score
      cor_test <- cor.test(group_expr, rep(ref_b_score, length(group_expr)), method = "spearman")
    ...
    if ("Response" %in% names(results_list) && "Non_response" %in% names(results_list)) {
      ...
      plots[[paste0(patient_id, "_Response")]] <- plotCorrelationWithStratifiedScore(
        gene_expr = as.numeric(patient_data$expr_data[select_omics, patient_data$response_samples]),
        stratified_score = rep(ref_b_score, length(patient_data$response_samples)),
        patient_id = paste(patient_id, "Response"),
        group = "Response"
      )

      plots[[paste0(patient_id, "_Non_response")]] <- plotCorrelationWithStratifiedScore(
        gene_expr = as.numeric(patient_data$expr_data[select_omics, patient_data$non_response_samples]),
        stratified_score = rep(ref_b_score, length(patient_data$non_response_samples)),
      ...
      correlation_data[[patient_id]] <- data.frame(
        ...
        p_value = wilcox.test(c(resp_data$cor, non_resp_data$cor),
                             c(rep("Response", length(resp_data$cor)),
                                rep("Non_response", length(non_resp_data$cor))))$p.value,
